...
@section('content')
    <div class="container my-5">
        <div class="card mx-auto" style="max-width: 800px;">
            <div class="card-header bg-warning text-white d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('teacher/bimbingan.show', $logbook->id) }}" class="btn btn-light">Kembali</a>
            </div>
            <div class="card-body mt-2">
                    ...
                    <div class="col-md-8">
                        @if ($logbook->foto_kegiatan)
                            <img src="{{ asset('storage/public/foto-kegiatan/' . $logbook->foto_kegiatan) }}"
                                height="200" width="200" class="img-fluid rounded" alt="Foto Kegiatan"
                ...
                <div class="row mb-3">
                    <div class="col-md-4"><strong>Penilaian Instruktur</strong></div>
                    <div class="col-md-8">
                        {{ $noteInstruktur ? $noteInstruktur->penilaian : 'Belum diisi' }}
                    </div>
                </div>
            </div>
            @if (empty($noteInstruktur->catatan))
    ...
    @if (empty($noteInstruktur->catatan))
        ...
        <div id="commentForm" class="card mx-auto mt-4" style="max-width: 800px; display: none;">
            <div class="card-body">
                <form action="{{ route('instructor/bimbingan.store') }}" method="POST">
                    @csrf
                    <input type="hidden" name="logbook_id" value="{{ $logbook->id }}">
                    <input type="hidden" name="note_type" value="INSTRUKTUR">
    ...
    @else
        ...
        <div id="EditcommentForm" class="card mx-auto mt-4" style="max-width: 800px; display: none;">
            <div class="card-body">
                <form action="{{ route('instructor/bimbingan.update', $noteInstruktur->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-3">
                        <label for="catatan" class="form-label"><strong>Komentar</strong></label>
                        <textarea name="catatan" id="editCatatan" class="form-control" rows="4">{{ old('catatan', $noteInstruktur->catatan) }}</textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="penilaian" id="editPenilaian" class="form-select">
                            @foreach ($grades as $grade)
                                <option value="{{ $grade }}"
                                    {{ $noteInstruktur->penilaian == $grade ? 'selected' : '' }}>
                                    {{ $grade }}
                                </option>
                            @endforeach
                        </select>
