...
@section('content')
    <div class="card p-4">
        ...
        <h5 class="card-header p-0 mb-4">
            Data Guru
            <span><a href="{{ route('admin/teacher.create') }}" class="text-decoration-none mx-2"><i
            ...
            <table id="example" class="table">
                <thead>
                    <tr class="text-nowrap bg-danger">
                        ...
                        <th class="text-white">Bidang Studi</th>
                ...
                <tbody>
                    @foreach ($teachers as $teacher)
                        <tr>
                            ...
                            <td>{{ $teacher->bidang_studi }}</td>
                            <td>{{ $teacher->pendidikan_terakhir }}</td>
                            <td>{{ $teacher->jabatan }}</td>
                            <td>{{ $teacher->tempat_lahir }}</td>
                            <td>{{ Carbon::parse($teacher->tanggal_lahir)->format('d M Y') }}</td>
                            <td>{{ $teacher->alamat }}</td>
                            <td>{{ $teacher->hp }}</td>
                            <td><img src="{{ asset('storage/public/teachers-images/' . $teacher->foto) }}"
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{ route('admin/teacher.edit', $teacher) }}">
